...
//    UserMapper userMapper;
    ...
    @Pointcut("@annotation(com.gantang.common.annotation.SysLog)")
    public void webLog() {
    }
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        User user = (User) redisUtil.get(request.getHeader("token"));
//        String openId = user.getOpenid();
        ...
        sysLog.setOpIp(ip);
        sysLog.setApiUrl(request.getRequestURL().toString());
//        if(session != null){
...
//        }
        ...
        com.gantang.common.annotation.SysLog mylog = method.getAnnotation(com.gantang.common.annotation.SysLog.class);
        if (mylog != null) {
            //注解上的描述
            sysLog.setOpDescribe(mylog.value());
        }
...
//        sysLog.setType(ToolUtil.isAjax(request)?"Ajax请求":"普通请求");
        if (user != null) {
            sysLog.setOpUser(user.getId() + "");
        }
        sysLog.setOpTime(DateUtil.getStringDate().toString());
...
//        }
        ...
        sysLog.setUseTime(DateUtil.formattime(System.currentTimeMillis() - startTime.get()));
//        userMapper.saveLogByAnnoation(sysLog);
    }
}